...
/** Seed the deterministic APP-01 "Application submitted" notification for UT. */
export function ensureApplicantApplicationSubmittedNotification(
  email: string,
  ...
  try {
    const existing = loadNotifications();
    if (existing.some((notification) => notification.id === APPLICANT_SUBMITTED_NOTIFICATION_ID)) return;

    const notification: AppNotification = {
      id: APPLICANT_SUBMITTED_NOTIFICATION_ID,
      forRole: 'applicant',
      forEmail: email,
...
/** Seed the deterministic INT-07 "Interview completed" notification for UT. */
export function ensureApplicantInterviewCompletedNotification(
  email: string,
  projectName: string,
  mentorName: string,
  href = '/apply/applications/app-ui-2027',
  ...
  try {
    const existing = loadNotifications();
    if (existing.some((notification) => notification.id === APPLICANT_INTERVIEW_COMPLETED_NOTIFICATION_ID)) return;

    const notification: AppNotification = {
      id: APPLICANT_INTERVIEW_COMPLETED_NOTIFICATION_ID,
      forRole: 'applicant',
      forEmail: email,
      title: 'Interview completed',
      body: `Thank you for meeting with ${mentorName} to discuss the ${projectName} opportunity. The internship team will review the outcome next. No action is required from you at this time.`,
      href,
      ctaLabel: 'View application',
      tier: 'info',
      createdAt: new Date().toISOString(),
