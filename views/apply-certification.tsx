      ...
      <div className="relative mx-[calc(-1*clamp(24px,2.6vw,40px))] min-h-[calc(100vh-64px)] bg-bg-subtle">
        ...
        <div className="mx-auto w-full max-w-[1440px] px-[clamp(24px,2.6vw,40px)] py-8">
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1.6fr)_minmax(280px,0.8fr)]">
            <Card className="overflow-hidden border-success/30 shadow-none">
              <div className="h-1 bg-success" aria-hidden />
              <CardContent className="p-7">
                <div className="flex items-start justify-between gap-4"><div><p className="text-[12px] font-medium uppercase tracking-[0.08em] text-fg-muted">University Internship 2026</p><h2 className="mt-2 text-[22px] font-semibold text-fg">Certificate of Completion</h2></div><Badge variant="success">Available</Badge></div>
                <div className="mt-6 flex items-center gap-4 rounded-lg bg-bg-muted p-5"><span className="flex size-12 items-center justify-center rounded-lg bg-surface text-accent"><Award className="size-6" aria-hidden /></span><div><p className="text-[14px] font-medium text-fg">Jenny Aw</p><p className="mt-1 text-[13px] text-fg-muted">Cybersecurity Threat Analysis · Issued 21 Sep 2026</p></div></div>
                <Alert variant="success" className="mt-5"><CheckCircle2 aria-hidden /><AlertDescription>Internship completed, required feedback submitted and final clearance approved.</AlertDescription></Alert>
                <div className="mt-6 flex flex-wrap gap-2"><Button onClick={() => router.push('/apply/applicant-certificate-viewer')}>View Certificate</Button><Button variant="outline" onClick={() => router.push('/apply/internship')}>View Internship Record</Button></div>
              </CardContent>
            </Card>
