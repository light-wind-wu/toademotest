...
import { GettingStartedPopover } from '@/components/ui-legacy/getting-started-popover';
import type { GettingStartedStep } from '@/components/ui-legacy/getting-started-popover';
import {
  Eye, AlertCircle, ArrowLeftToLine, Info, Lightbulb,
...
/* ── Demo data helpers for the how-to page (no persistence) ──────────── */
...
function demoRequestBase(): ReqEntry {
  const programmeCentre = 'PC3';
  return {
    id: 0,
    department: departmentForProgrammeCentre(programmeCentre),
    programmeCentre,
    pcHead: pcHeadForProgrammeCentre(programmeCentre),
    adpnc: adPncForProgrammeCentre(programmeCentre),
    deadline: lastDayOfCurrentMonth(),
    levels: [emptyLevel()],
    open: true,
...
function demoRequestFull(): ReqEntry {
  const base = demoRequestBase();
  const start = '2027-05-01';
  const end = '2027-08-31';
  const level: ReqLevel = {
    level: 'Undergraduate Student',
    calendarStart: start,
    calendarEnd: end,
    calendarPeriod: monthRangeLabel(start, end),
    duration: '2 Months',
    placements: 2,
  };
  return { ...base, levels: [level] };
}
...
/* ── Main page ───────────────────────────────────────────────────── */
export default function ProjectRequestHowToPage() {
  ...
  /* Start with the base demo request. */
  useEffect(() => {
    setReqs([{ ...demoRequestBase(), id: idRef.current++ }]);
  }, []);

  function handleStepClick(stepNumber: GettingStartedStep) {
    if (stepNumber === 1) {
      setReqs([{ ...demoRequestBase(), id: idRef.current++ }]);
      setStep(1);
    } else if (stepNumber === 2) {
      setReqs([{ ...demoRequestFull(), id: idRef.current++ }]);
      setStep(1);
    } else {
      setReqs([{ ...demoRequestFull(), id: idRef.current++ }]);
      setStep(2);
  ...
  return (
    <TooltipProvider>
      <Shell activeRoute="/requests/how-to-get-started">
      <div className="flex min-h-[calc(100vh-112px)] flex-col">
        ...
        <div className="mb-4 flex shrink-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <Breadcrumb className="text-label-md">
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>How to get started</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
          <GettingStartedPopover onStepClick={handleStepClick} />
        </div>
        ...
        {step === 1 && (
          <Alert variant="default" className="mb-6 shrink-0 bg-[#F3EFE5] border-[#E7E4DD]">
            <Lightbulb size={16} className="shrink-0 text-fg-muted" />
            <AlertDescription>
        ...
        {step === 2 && (
          <div className="flex-1">
            <Alert variant="default" className="mb-4 bg-[#F3EFE5] border-[#E7E4DD]">
              ...
              <AlertDescription>
                <span className="font-semibold">Review before sending:</span>{' '}
                Please review the request details below before sending. You can save as draft if you need to make changes later.
              </AlertDescription>
            </Alert>
            {buildLayout === 'Layout 1' ? (
              <ReviewListLayout
          ...
          <div className="flex items-center gap-3">
            {step === 1 ? (
              ...
              <>
                ...
                <Button variant="outline" size="md" onClick={handleSaveDraft}>Save as Draft</Button>
                <Button size="md" onClick={() => setDemoNoticeOpen(true)}>Confirm Send</Button>
              </>
            )}
          </div>
        </div>
      ...
      <Dialog open={demoNoticeOpen} onOpenChange={setDemoNoticeOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              This is a demo environment. No data will be saved.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button onClick={() => setDemoNoticeOpen(false)}>
              OK
            </Button>
          </DialogFooter>
        </DialogContent>
