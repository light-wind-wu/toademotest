...
import { loadApplicantOfferEmail, markApplicantOfferEmailRead } from '@/lib/applicant-offer';
import type { ApplicantOfferEmail } from '@/lib/types';
import { cn } from '@/lib/utils';

const mailFolders = [
  { label: 'Inbox', icon: Inbox, count: 980, active: true },
...
export default function GmailOffer() {
  const router = useRouter();
  const [sidebarOpen, setSidebarOpen] = useState(true);
  const [message, setMessage] = useState<ApplicantOfferEmail | null>(null);

  useEffect(() => {
    const email = loadApplicantOfferEmail();
    setMessage(email);
    if (email) markApplicantOfferEmailRead();
  }, []);
  ...
  return (
    <main className="min-h-[100dvh] bg-[#f6f8fc] font-sans text-[#1f1f1f]">
      <header className="sticky top-0 z-20 flex h-16 items-center gap-3 bg-[#f6f8fc] px-3 md:px-5">
        ...
        <div className="ml-auto flex items-center gap-1"><IconButton label="Help"><CircleHelp className="size-5" aria-hidden /></IconButton><IconButton label="Settings"><Settings className="size-5" aria-hidden /></IconButton><button type="button" onClick={() => router.push('/desktop')} aria-label="Return to Desktop" className="ml-1 inline-flex h-9 items-center gap-2 rounded-full px-3 text-[13px] font-medium text-[#3c4043] hover:bg-[#e8eaed]"><MonitorUp className="size-[18px]" aria-hidden /><span className="hidden lg:inline">Return to Desktop</span></button><span className="ml-1 flex size-9 items-center justify-center rounded-full bg-[#c5221f] text-[14px] font-semibold text-white">JA</span></div>
      </header>
        ...
        <section className="min-w-0 flex-1 overflow-hidden rounded-2xl bg-white shadow-[0_1px_2px_rgba(60,64,67,0.08)]">
          ...
          {message ? (
            <article className="mx-auto max-w-[980px] px-4 pb-12 pt-6 sm:px-8 lg:px-12">
              ...
              <div className="mt-5 flex items-start gap-3"><span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-[#e8eaed] text-[13px] font-medium">DT</span><div className="min-w-0 flex-1"><div className="flex flex-wrap items-baseline gap-x-2"><p className="text-[14px] font-semibold">{message.senderName}</p><p className="truncate text-[12px] text-[#5f6368]">&lt;{message.senderEmail}&gt;</p><time className="ml-auto text-[12px] text-[#5f6368]" dateTime={message.receivedAt}>Sat 29 Aug, 09:15</time></div><button type="button" className="mt-0.5 flex items-center gap-1 text-[12px] text-[#5f6368]">to me <ChevronDown className="size-3" aria-hidden /></button></div></div>
              <div className="mx-auto mt-8 max-w-[700px] border border-[#dadce0] bg-white px-7 py-9 sm:px-14 sm:py-12">
                ...
                <div className="mt-6 space-y-4 text-[14px] leading-6 text-[#3c4043]"><p>We are pleased to offer you an internship under the <strong className="text-[#202124]">{message.programmeName}</strong> for <strong className="text-[#202124]">{message.projectName}</strong>.</p><p>Please review the internship period, reporting location and offer terms in the Applicant Portal. Submit your response by <strong className="text-[#202124]">{message.responseDeadline}</strong>.</p></div>
                <dl className="mt-6 grid gap-4 rounded-lg bg-[#f1f3f4] p-5 text-[13px] sm:grid-cols-2"><div className="flex gap-2"><CalendarDays className="mt-0.5 size-4 text-[#5f6368]" aria-hidden /><div><dt className="text-[#5f6368]">Internship period</dt><dd className="mt-1 font-semibold text-[#202124]">{message.internshipPeriod}</dd></div></div><div className="flex gap-2"><MapPin className="mt-0.5 size-4 text-[#5f6368]" aria-hidden /><div><dt className="text-[#5f6368]">Reporting location</dt><dd className="mt-1 font-semibold text-[#202124]">{message.reportingLocation}</dd></div></div><div><dt className="text-[#5f6368]">Work arrangement</dt><dd className="mt-1 font-semibold text-[#202124]">{message.workArrangement}</dd></div><div><dt className="text-[#5f6368]">Allowance</dt><dd className="mt-1 font-semibold text-[#202124]">{message.allowance}</dd></div></dl>
                <button type="button" onClick={() => router.push('/apply/applicant-offer-detail?applicationId=app-ui-2027')} className="mt-6 inline-flex h-10 items-center rounded-md bg-[#0b57d0] px-5 text-[13px] font-semibold text-white hover:bg-[#0842a0]">Review offer</button>
                <p className="mt-5 text-[14px] leading-6 text-[#3c4043]">We hope you will join us at DSTA.</p>
                <div className="mt-10 text-[13px] leading-5 text-[#5f6368]"><p>Warm regards,</p><div className="mt-4 flex items-center gap-4 border-t border-[#e0e0e0] pt-4"><Image src="/images/dsta-logo.svg" alt="DSTA" width={88} height={48} style={{ width: 88, height: 'auto' }} /><div><p className="font-semibold text-[#202124]">Davina Tan</p><p>Internship Officer, DSTA Talent Acquisition</p></div></div></div>
              </div>
          ...
          ) : <div className="flex min-h-[520px] flex-col items-center justify-center px-6 text-center"><Mail className="size-7 text-[#0b57d0]" aria-hidden /><h1 className="mt-4 text-[20px] font-medium">No offer email yet</h1><p className="mt-2 max-w-md text-[14px] leading-6 text-[#5f6368]">The simulated offer email will arrive after the interview outcome is completed and the IO issues the offer.</p><button type="button" onClick={() => router.push('/desktop')} className="mt-5 rounded-full bg-[#0b57d0] px-5 py-2.5 text-[14px] font-medium text-white">Return to Desktop</button></div>}
        </section>
      </div>
    </main>
  );
